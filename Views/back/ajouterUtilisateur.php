...
if (
    ...
    $prenom_user = htmlspecialchars(trim($_POST['prenom_user']));
    $email_user = htmlspecialchars(trim($_POST['email_user']));
    $tel_user = htmlspecialchars(trim($_POST['tel_user']));
    $adresse_user = htmlspecialchars(trim($_POST['adresse_user']));
    $username = htmlspecialchars(trim($_POST['username']));
    $password_user = trim($_POST['password_user']);
    $role_user = htmlspecialchars(trim($_POST['role_user']));

    // Vérifications des champs
    if (empty($nom_user) || strlen($nom_user) < 2) {
        $errors[] = "Le nom doit contenir au moins 2 caractères.";
    }
    if (empty($prenom_user) || strlen($prenom_user) < 2) {
        $errors[] = "Le prénom doit contenir au moins 2 caractères.";
    }
    if (!filter_var($email_user, FILTER_VALIDATE_EMAIL)) {
        $errors[] = "L'email est invalide.";
    }
    if (!preg_match('/^[0-9]{8,15}$/', $tel_user)) {
        $errors[] = "Le numéro de téléphone doit contenir entre 8 et 15 chiffres.";
    }
    if (empty($adresse_user) || strlen($adresse_user) < 5) {
    ...
    if (!in_array($role_user, ['admin', 'user'])) { // Exemple de contrôle pour le rôle
        $errors[] = "Le rôle sélectionné est invalide.";
    }

    // Si aucune erreur, continuer
    if (empty($errors)) {
        $password_user = md5($password_user); // Hashage du mot de passe
